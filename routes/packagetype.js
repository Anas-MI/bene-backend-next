...
router.get("/getAll", async function (req, res) {
  Packagetype.find({})
    .then((response) => res.status(200).json({ package: response }))
...
router.post("/edit/:id", async function (req, res) {
  ...
  async function show(data) {
    ...
    if (errors) {
      ...
      let isPackagetypeid = await Packagetype.find({
        packingid: req.body.packingid,
      });
      if (isPackagetypeid.length == 0) {
        Packagetype.findByIdAndUpdate(id, req.body, { new: true })
          .then((data) => res.redirect("/packagetype/all"))
          .catch((err) => res.json({ status: false, error: err }));
        ...
        res.render("edit_Packagetype.hbs", {
          error: [
            { param: "error", msg: "Package Id already exists", value: "" },
          ],
        });
      }
    }
...
router.post("/editPackageType/:id", async function (req, res) {
  let id = req.params.id;
  Packagetype.findById(id)
    .then((data) => show(data))
    .catch((err) => res.json({ status: false, error: err }));
  async function show(data) {
    ...
    if (isPackagetypeid.length == 0) {
      ...
      return res
        .status(404)
        .json({ message: "Package Type ID Already exists!" });
    }
    // }
  }
});
...
router.post("/addPackageType", async function (req, res) {
  req.checkBody("packingid", "Packing ID is required").notEmpty();
  req.checkBody("packingtype", "Packing Type is required").notEmpty();
  req.checkBody("description", "Description is required").notEmpty();
  let errors = req.validationErrors();
  ...
  if (isPackagetypeid.length == 0) {
    ...
    return res.status(404).json({ message: "Package Type ID Already exists!" });
  }
  // }
});
...
router.get("/delete/:id", async function (req, res) {
  ...
  Packagetype.findByIdAndRemove(id)
    .then(() => res.redirect("/packagetype/all"))
    .catch((err) => res.status({ error: err }));
});

router.delete("/deletePackageType/:id", async function (req, res) {
  let id = req.params.id;
  Packagetype.findByIdAndRemove(id)
    .then(() =>
