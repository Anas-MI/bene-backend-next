...
router.post("/add", async function (req, res) {
  ...
  allFooters = await Footer.find({ parentid: parentId })
    .select("-1")
    .sort({ _id: "asc" });
  ...
  if (req.body.reactData) {
    ...
    await footer
      .save()
      .then((footer) => res.redirect("/footermenus/getAllFooters"))
      .catch((err) => {
        console.log(err.message);
        res
          .status(404)
          .json({ success: false, message: "Validation error", errors: err });
      });
  } else {
    await footer
      .save()
      .then((footer) => res.redirect("/footermenus/all"))
      .catch((err) => {
        console.log(err.message);
        res
          .status(404)
...
router.post("/edit/:id", async function (req, res) {
  ...
  async function customcheck(keytest) {
    ...
    async function check(data) {
      ...
      Footer.findOneAndUpdate({ _id: query }, footermenu, {
        new: true,
      })
        .then((data) => {
          if (req.body.reactData) {
            return res.redirect("/footermenus/getAllFooters");
          } else {
            return res.json({
              success: true,
        ...
        .catch((err) =>
          res.status(404).json({ message: "Error Editing Footer Menu" })
        );
    }
  }
});
...
router.get("/getall", async function (req, res) {
  ...
  function sort(data) {
    if (data.length > 1) {
      ...
      res.setHeader(
        "Cache-Control",
        "Vary: Accept-Encoding Header, max-age=31557600"
      );
...
router.get("/deleteFooterMenu/:id", async function (req, res) {
  let id = req.params.id;
  Footer.findOneAndRemove({ _id: id }).then((data) => {
    res.redirect("/footermenus/getAllFooters");
  });
});

router.get("/all", async function (req, res) {
  let myfooter = await Footer.find({ parentid: { $exists: false } })
    .select("-1")
    .sort({ _id: "asc" });
  let newfooters = [];
  let childMenus = await Footer.find({ parentid: { $exists: true } })
    .select("-1")
    .sort({ footerorder: "asc" });
  var result = myfooter
    .map(function (el) {
      let footerID = el._id;
      const mychild = childMenus
        .map((childitem, index) => {
  ...
  Footer.find({}, function (err, footer) {
    if (err)
      return res.status(404).render("all_footer.hbs", {
...
router.get("/getAllFooters", async function (req, res) {
...
router.post("/change-order", async function (req, res) {
  ...
  menus.map((menuid, index) => {
    ...
    Footer.findByIdAndUpdate(query, menu, { new: true })
  ...
  if (errorArray.length == 0) {
    return res.status(200).json({
      success: true,
      message: "Footer Order Saved Successfully",
      body: req.body,
    });
  } else {
    return res.status(200).json({
      success: false,
      message: "Error in Saving Footer Order",
      errors: errorArray,
    });
  }
});
