...
router.post("/api/login", async function (req, res) {
  if (req.body.facebook) {
    ...
    if (!checkIfExists) {
      User2.findOne({ facebook: req.body.facebook }).then((data) =>
        check(data)
      );
...
router.post("/api/profile/password", async function (req, res) {
  ...
  async function set(data) {
    ...
    if (data.password == encrypted) {
      let cipher = crypto.createCipheriv(algorithm, new Buffer.from(key), iv);
      var encrypted2 =
        cipher.update(password2, "utf8", "hex") + cipher.final("hex");
      await User2.findOneAndUpdate(
        { _id: id },
...
router.get("/usermanagement/delete/:id", async (req, res) => {
  ...
  User2.findByIdAndRemove({ _id: usermetaid }, function (err, user) {
    Usermeta2.findByIdAndDelete({ _id: userid }, function (err, product) {
      if (err) {
        res.status(404).json({ messsgae: err.message });
      }
...
router.get("/getUsers", async function (req, res) {
  User2.find({})
    ...
    .catch((err) => res.status(404).json({ message: "Error fetching users" }));
});
...
router.post("/login", function (req, res, next) {
  ...
  let errors = req.validationErrors();
  if (errors)
    return res.status(404).res.render("login.hbs", {
      pageTitle: "Login",
...
router.post("/api/forgetpassword", async function (req, res, next) {
  if (req.body.firststep) {
    ...
    let userExist = await User2.findOne(
      { email: req.body.email },
      "email",
      function (err, user) {}
    );
    if (!userExist) {
      ...
      var url =
        process.env.serverurl + "/users/forgetpassword/?token=" + userExist._id;
    }
