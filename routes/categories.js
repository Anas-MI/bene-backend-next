...
router.post("/add", upload.single("categoryimage"), (req, res, next) => {
  ...
  req
    .checkBody("categorydescription", "Category Description is required")
    .notEmpty();
  ...
  category.save(function (err, data) {
    if (err) {
      ...
      return res
        .status(200)
        .json({ success: true, message: "Category Added", category: data });
    }
  });
  //   category
  //     .save()
...
router.get("/all", ensureAuthenticated, function (req, res) {
  ...
  Category.find({}, function (err, categories) {
    if (err)
      return res.status(404).render("all_category.hbs", {
...
router.get("/getcategories", function (req, res) {
  Category.find({}, function (err, categories) {
    if (err) {
      return res.status(404).json({
        success: false,
        message: "Error in fetching Categorys Record",
      });
    } else {
...
router.post("/edit/:id", upload.single("brandimage"), function (req, res) {
  ...
  Category.findOneAndUpdate(query, category, { new: true }, (err, doc) => {
    if (err) {
      return res.status(404).json({
        success: false,
  ...
  //   Category.update(query, category, function (err, data) {
  //     if (err) {
  //       return res.status(404).json({
  //         success: false,
  //         message: "Error in Updating Category",
