...
router.post("/add", upload.any(), async function (req, res) {
  ...
  if (errors) {
    ...
    return res
      .status(404)
      .json({ success: false, message: "validation error", error: errors });
  }
  ...
  let pageExist = await Page.findOne({ title: req.body.title })
    .select("-1")
    .sort({ _id: "asc" });
  if (pageExist) {
    res.json({
      ...
      serialerror: "Page already exist with this Title",
    });
  } else {
    if (req.body.titlelayout1) {
      let optionsData = {};
      ...
      items["secondsection"] = {
        title: req.body.secondtitle,
        description: req.body.seconddescription,
        visibility: req.body.secondvisibility,
        linktext: req.body.secondlinktext,
        btnlink: req.body.secondlink,
      };

      items["thirdsection"] = {
        stitle: req.body.thirdstitle,
        title: req.body.thirdtitle,
        description: req.body.thirddescription,
        visibility: req.body.thirdvisibility,
        btntext: req.body.thirdbtntext,
        btnlink: req.body.thirdbtnlink,
        imagelink: optionsData.thirdimage,
      };

      items["fourthsection"] = {
        stitle: req.body.fourthstitle,
        title: req.body.fourthtitle,
        description: req.body.fourthdescription,
        visibility: req.body.fourthvisibility,
        btntext: req.body.fourthbtntext,
        btnlink: req.body.fourthbtnlink,
        imagelink: optionsData.fourthimage,
      };

      items["fifthsection"] = {
        stitle: req.body.fifthstitle,
        title: req.body.fifthtitle,
        description: req.body.fifthdescription,
        visibility: req.body.fifthvisibility,
        btntext: req.body.fifthbtntext,
        btnlink: req.body.fifthbtnlink,
        imagelink: optionsData.fifthimage,
      };

      items["sixthsection"] = {
        title: req.body.sixthtitle,
        description: req.body.sixthdescription,
        visibility: req.body.sixthsectionvisibility,
      };
...
router.post("/continent/add/:id", function (req, res) {
  ...
  Page.findByIdAndUpdate(id, { $push: { continent: req.body.continent } }).exec(
    function (err, pages) {
      if (err) {
        console.log(err);
      } else {
...
router.post("/edit/:id", upload.any(), async (req, res) => {
  ...
  if (req.body.titlelayout1) {
    ...
    if (
      optionsData.thirdcopyimage_1 === undefined ||
      optionsData.thirdcopyimage_1 === null
    ) {
      optionsData.thirdcopyimage_1 = req.body.thirdcopyimagelink_1;
    }
    ...
    if (
      optionsData.thirdcopyimage_2 === undefined ||
      optionsData.thirdcopyimage_2 === null
    ) {
      optionsData.thirdcopyimage_2 = req.body.thirdcopyimagelink_2;
    }
    ...
    var image21 = "";
    if (optionsData.secondimage21) {
      image21 = optionsData.secondimage21;
    ...
    Page.findOneAndUpdate(query, page, function (err) {
      if (err) {
        res.status(404).json({
          success: false,
          message: "Error in Updating Page",
          errors: err,
        });
      } else {
        res.status(200).json({ success: true, message: "Page Updated" });
    ...
    if (
      optionsData.firstbgimage === undefined ||
      optionsData.firstbgimage === null
    ) {
      optionsData.firstbgimage = req.body.firstimagelink;
    }
...
router.get("/pagecontent", async function (req, res) {
  ...
  function check(data) {
    if (data.length !== 0) {
      ...
      Page.find()
        .then((data) =>
          res.json({ status: false, error: "country not found", content: data })
        )
...
router.post("/update", function (req, res) {
  ...
  Page.findByIdAndUpdate(id, { $set: { continent } }, { new: true })
    .then((data) => res.status(200).json({ status: true, data }))
...
router.get("/orders/cancelled/all", ensureAuthenticated, function (req, res) {
  OOrder.find({
    $or: [{ deleted: "Cancelled By Admin" }, { deleted: "Cancelled By User" }],
  })
    .sort({ _id: "desc" })
    .exec(function (err, orders) {
      if (err) {
...
router.get("/orders/delete/:id", async function (req, res) {
  ...
  OrderProduct.findOneAndUpdate(
    { _id: id },
    { $set: { deleted: "Cancelled By Admin" } }
  )
...
router.get("/deleteOrder/:id", async function (req, res) {
  ...
  OOrder.findOneAndUpdate(
    ...
    .catch((err) =>
      res.status(404).json({ message: "Problem Deleting Order" })
    );
});
