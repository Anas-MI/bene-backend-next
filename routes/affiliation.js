    ...
    if (await Affiliate.findOne({ email: req.body.email })) {
      ...
      async function setdata(user) {
        ...
        var mailOptions = {
          ...
          text:
            "You have been successfully registered as an Ambassador on CBDBene. You can log into the Ambassador Portal once we approve you. Keep an eye on your inbox.",
        };
...
router.post("/add/url", async function (req, res) {
  ...
  if (await isvalid) {
    ...
    referral
      .save()
      .then((data) => addit(data))
      .catch((err) => res.status(400).json({ status: false, error: err }));
    function addit(referral) {
      Affiliate.findOneAndUpdate(
        { ambass_id },
        { $push: { urlvisits: referral._id } },
        { new: true }
      )
        .populate("urlvisits")
        .then((data) => res.status(200).json({ status: true, data, referral }))
        .catch((err) => res.status(400).json({ status: false, error: err }));
    }
  } else {
...
router.post("/convert/:id", async function (req, res) {
  ...
  Referral.findByIdAndUpdate(
    id,
    { $set: { converted: true, orderid, amount } },
    { new: true }
  )
...
router.post("/update", async function (req, res) {
  ...
  Affiliate.findByIdAndUpdate(id, affl, { new: true })
...
router.get("/getAmbassadors", async function (req, res) {
  Affiliate.find({}).then((data) => show(data));
  function show(data) {
    res.status(200).json({
      user: data,
...
router.get("/viewAmbassador/:id", async function (req, res) {
  let query = req.params.id;
...
router.post("/register/admin", async function (req, res) {
  if (await Affiliate.findOne({ email: req.body.email })) {
    ...
    if (req.body.reactData) {
      ...
      affiliate
        .save()
        .then((data) => res.redirect("/ambassador-portal/usermanagement"))
        .catch((err) => res.json({ status: false, error: err }));
    }
  }
});
...
router.get("/approve/:id", ensureAuthenticated, (req, res) => {
  let id = req.params.id;
  Affiliate.findByIdAndUpdate(id, { $set: { status: true } }).then((data) =>
    sendmail(data)
  );
  ...
  function sendmail(data) {
    var mailOptions = {
      ...
      text:
        "We’re so happy to officially welcome you to the CBD Bene Ambassador Team. You can login to our portal at any time to view your activity, track commissions, share and more.",
    };
...
router.delete("/delete/:id", (req, res) => {
  Affiliate.findById(req.params.id)
    ...
    .catch((err) => res.status(404).json("Error Deleting Ambassador"));
});

router.get("/approveAmbassador/:id", (req, res) => {
  let id = req.params.id;
  console.log(id);
...
router.get("/statistics", async (req, res) => {
  ...
  let productsSold = productsArr
    .map((products) => {
      console.log(products.converted);
      if (!products.comboId) {
        return products;
      }
    })
  ...
  let totalLinks = referral
    .map((links) => {
      if (links.converted === true) return links;
    })
    .filter((notUndefined) => notUndefined !== undefined);
  ...
  let bestSelling = productsSold.map((bestSelling) => {
    return bestSelling.title;
  });

  console.log(bestSelling);
  let count1 = {};
  bestSelling.forEach(function (i) {
    count1[i] = (count1[i] || 0) + 1;
  });
  console.log({ count1 });

  //Best selling kit
  let bestSellingKit = kitsSold.map((bestSelling) => {
    return bestSelling.title;
  });
  let count2 = {};
  bestSellingKit.forEach(function (el) {
  ...
  res.render("statistics.hbs", {
    visits,
    productsSold: productsSold1,
    kitsSold: kitsSold.length,
