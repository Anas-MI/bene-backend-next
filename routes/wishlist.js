    ...
    function check(data) {
      if (data.length > 0) {
        ...
        wishlist
          .save()
          .then((wishlist) =>
            res.status(200).json({ status: true, wishlist: wishlist })
          );
      }
    }
  } else {
...
router.post("/api/delete", function (req, res) {
  ...
  if (errors) {
    ...
    Wishlist.findOneAndRemove({ _id: query })
      .then(() =>
        res.status(200).json({ success: true, message: "Succesfully removed" })
      )
...
router.post("/api/byuser", function (req, res) {
  ...
  Wishlist.find({ userid: user, combo: false })
    .populate("productid")
    .populate("productmeta")
    .then((wishlist) => display(wishlist));
  function display(data) {
    Wishlist.find({ userid: user, combo: true })
      .populate("comboid")
      .then((data1) =>
        res.status(200).json({ success: true, wishlist: data, combo: data1 })
      )
      .catch((err) => res.status(400).json({ success: false, error: err }));
  }
});
...
router.get("/all", async function (req, res) {
  let wishlist = await Wishlist.find({ combo: false })
    .populate("productid")
    .populate("userid")
    .populate("productmeta")
    .select("-1")
    .sort({ _id: "asc" });
  let combos = await Wishlist.find({ combo: true })
    .populate("comboid")
    .populate("userid");
...
router.get("/allWishlist", async function (req, res) {
  let wishlist = await Wishlist.find({ combo: false })
    .populate("productid")
    .populate("userid")
    .populate("productmeta")
...
router.post("/edit/:id", async function (req, res) {
  ...
  Wishlist.findOneAndUpdate({ _id: query }, newWishlist)
    .then(() => res.redirect("/wishlist/all"))
...
router.get("/deleteWishlist/:id", async function (req, res) {
  let query = req.params.id;

  Wishlist.findOneAndRemove(query, function (err, product) {
    if (err) {
      res.status(404).json({ message: "Error Deleteing Wishlist" });
    }
