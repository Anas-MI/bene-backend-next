...
router.post("/coupon/generate", async (req, res) => {
  ...
  if (req.body.duration === "repeating") {
    data.duration_in_months = req.body.months;
  }
  stripe.coupons.create(data, function (err, coupon) {
    if (err) {
...
router.get("/admin/coupon/list/:id", (req, res) => {
  ...
  console.log(name);
  stripe.coupons.del(name, function (err, confirmation) {
    if (err) {
      console.log(err);
    } else {
    ...
    async function (err, subscription) {
      if (err) {
        ...
        if (req.body.userid) {
          await Usermeta2.findOneAndUpdate(
            { userid: req.body.userid },
            { $set: { stripe_subscription: subscription } }
          );
        }
...
router.get("/listplans", async (req, res) => {
  stripe.plans.list({ limit: 1000 }, function (err, plans) {
    if (err) {
      res.json({ status: true, error: err });
    } else {
...
router.get("/getplan/:id", async (req, res) => {
  stripe.plans.retrieve(req.query.id, function (err, plan) {
    if (err) {
      res.json({ status: false, error: err });
    } else {
      res.json({ status: true, plan });
    ...
    if (err) {
      ...
      if (req.body.userid) {
        await Usermeta2.findOneAndUpdate(
          ...
          {
            $set: {
              stripe_plan: "cancelled",
              stripe_subscription: "cancelled",
            },
          }
        );
      }
