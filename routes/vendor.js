...
router.get("/getAll", async function (req, res) {
  Vendor.find({})
    .then((response) => res.status(200).json({ vendor: response }))
  ...
  async function show(data) {
    ...
    if (isvendorid.length == 0) {
      ...
      if (isvendorname.length == 0) {
        await Vendor.findByIdAndUpdate(id, req.body, { new: true })
          .then((data) => res.redirect("/vendor/all"))
          .catch((err) => res.json({ status: false, error: err }));
      } else {
...
router.post("/editVendor/:id", async function (req, res) {
  let id = req.params.id;
  Vendor.findById(id)
    .then((data) => show(data))
    .catch((err) => res.json({ status: false, error: err }));
  async function show(data) {
    ...
    if (isvendorid.length == 0) {
      ...
      if (isvendorname.length == 0) {
        ...
        return res.status(404).json({ message: "Vendor Name Already exists!" });
      }
    } else if (req.body.vendorid == data.vendorid) {
      await Vendor.findByIdAndUpdate(id, req.body, { new: true })
        .then((response) => res.status(200).json({ vendor: response }))
      ...
      return res.status(404).json({ message: "Vendor ID Already exists!" });
    }
  }
});
